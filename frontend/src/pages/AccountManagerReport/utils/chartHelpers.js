...
/**
 * Build tech stack pie chart data from report data
 * @param {Object} employeesByTechStack - { techStackName: count, ... }
 */
export const buildTechStackPieData = (employeesByTechStack = {}) => {
  const labels = Object.keys(employeesByTechStack);
  const data = Object.values(employeesByTechStack);

  // Use a color palette for tech stacks
  const colorPalette = [
    colors.primary,
    colors.secondary,
    colors.success,
...
/**
 * Tech stack pie chart options
 */
export const techStackPieOptions = {
  ...commonOptions,
  plugins: {
    ...commonOptions.plugins,
    legend: {
