 ...
 * Returns all external consultants with their projects, statistics, and distribution charts
 */
export const getExternalConsultantsReport = async (event) => {
    const log = logger.child({ handler: 'reports.getExternalConsultantsReport' });
    ...
    try {
        ...
        log.info('Getting external consultants report', { filters: queryParams });

        // Build WHERE clause for resources
        let resourceWhereClause = 'WHERE r.is_external_consultant = true AND r.status = \'Active\' AND r.deleted_at IS NULL';
        const params = [];
        let paramCounter = 1;

        if (track_id) {
            params.push(track_id);
            resourceWhereClause += ` AND r.track_id = $${paramCounter++}`;
        }

        if (tech_stack) {
            params.push(tech_stack);
            resourceWhereClause += ` AND r.tech_stack = $${paramCounter++}`;
        }

        // Build allocation filters for project-related filters
        let allocationWhereClause = `
            WHERE a.is_active = true 
        ...
        if (project_id) {
            ...
            allocationWhereClause += ` AND a.project_id = $${paramCounter++}`;
        }

        if (project_name) {
            params.push(`%${project_name}%`);
            allocationWhereClause += ` AND p.project_name ILIKE $${paramCounter++}`;
        }

        if (account_manager) {
            params.push(`%${account_manager}%`);
            allocationWhereClause += ` AND p.account_manager ILIKE $${paramCounter++}`;
        }

        if (start_date) {
            params.push(start_date);
            allocationWhereClause += ` AND a.start_date >= $${paramCounter++}`;
        }

        if (end_date) {
            params.push(end_date);
            allocationWhereClause += ` AND (a.end_date IS NULL OR a.end_date <= $${paramCounter++})`;
        }
        ...
        // Get total external consultants count
        const totalConsultantsQuery = project_name || account_manager || project_id
            ? `
                SELECT COUNT(DISTINCT r.id) as total
                FROM resources r
                INNER JOIN allocations a ON r.id = a.resource_id
        ...
        // Get external consultants with their projects (BY PROJECT table)
        const consultantsByProjectQuery = project_name || account_manager || project_id
            ? `
                SELECT 
                    r.id as resource_id,
                    r.employee_id,
                    ...
                    p.id as project_id,
                    p.project_name as project,
                    a.start_date as project_allocated_date,
                    a.end_date as project_deallocated_date,
                    a.allocation_percentage as project_allocation_percentage
                FROM resources r
        ...
        // Get distribution by tech stack
        const techStackDistributionQuery = `
            SELECT 
                COALESCE(r.tech_stack, 'Unspecified') as tech_stack,
                COUNT(*) as count
            FROM resources r
            ${resourceWhereClause}
            GROUP BY r.tech_stack
            ORDER BY count DESC, r.tech_stack
        `;

        // Execute all queries in parallel
        const [
            totalEmployeesResult,
            totalConsultantsResult,
            consultantsByProjectResult,
            trackDistributionResult,
            techStackDistributionResult
        ] = await Promise.all([
            db.query(totalEmployeesQuery),
            db.query(totalConsultantsQuery, params),
            db.query(consultantsByProjectQuery, params),
            db.query(trackDistributionQuery, params.filter((_, i) => i < (track_id ? 1 : 0))),
            db.query(techStackDistributionQuery, params.filter((_, i) => i < (track_id ? 1 : 0)))
        ]);

        const totalEmployees = parseInt(totalEmployeesResult.rows[0]?.total || 0);
        const totalConsultants = parseInt(totalConsultantsResult.rows[0]?.total || 0);
        const consultantsPercentage = totalEmployees > 0
            ? ((totalConsultants / totalEmployees) * 100).toFixed(2)
            : '0.00';

        // Format track distribution for charts
        const trackDistribution = trackDistributionResult.rows.map(row => ({
            track: row.track,
            count: parseInt(row.count),
            percentage: totalConsultants > 0
                ? ((parseInt(row.count) / totalConsultants) * 100).toFixed(2)
                : '0.00'
        }));

        // Format tech stack distribution for charts
        const techStackDistribution = techStackDistributionResult.rows.map(row => ({
            techStack: row.tech_stack,
            count: parseInt(row.count)
        }));

        // Format BY PROJECT table data
        const byProjectData = consultantsByProjectResult.rows.map(row => ({
            resourceId: row.resource_id,
            employeeId: row.employee_id,
            employeeName: row.employee_name,
            track: row.track,
            techStack: row.tech_stack || 'Unspecified',
            projectId: row.project_id,
            project: row.project || 'No Active Project'
        }));

        // Format BY ALLOCATION table data (with date details)
        const byAllocationData = consultantsByProjectResult.rows
            .filter(row => row.project_id) // Only include rows with actual projects
            .map(row => {
                const startDate = row.project_allocated_date ? new Date(row.project_allocated_date) : null;
                const endDate = row.project_deallocated_date ? new Date(row.project_deallocated_date) : null;

                let durationDays = null;
                if (startDate) {
                    const compareDate = endDate || new Date();
                    durationDays = Math.ceil((compareDate - startDate) / (1000 * 60 * 60 * 24));
                }
                ...
                return {
                    ...
                    projectId: row.project_id,
                    project: row.project,
                    projectAllocatedDate: row.project_allocated_date,
                    projectDeallocatedDate: row.project_deallocated_date,
                    projectAllocationPercentage: parseFloat(row.project_allocation_percentage) || 0,
        ...
        return success({
            summary: {
                totalEmployees,
                totalExternalConsultants: totalConsultants,
                externalConsultantsPercentage: parseFloat(consultantsPercentage)
            },
            charts: {
                trackDistribution,
